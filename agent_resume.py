...
class DQNAgent:
    ...
    def reloadModel(self):
        ...
        tf.compat.v1.disable_eager_execution()
        imported_graph = tf.compat.v1.train.import_meta_graph('./new_reward_models/model-2435923.meta')
        imported_graph.restore(self.session, './new_reward_models/model-2435923')

    def resumeTraining(self):
        self.reloadModel()
        self.step = 2435923
        self.eps = self.eps_decay**self.step
        self.burnin += self.step
    ...
    def run(self, state):
        ...
        if np.random.rand() < self.eps:
            ...
            # Policy action
            q = self.predict('target', np.expand_dims(state, 0))
            action = np.argmax(q)
        # Decrease eps
